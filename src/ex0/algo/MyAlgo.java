...
public class MyAlgo implements ElevatorAlgo{
    ...
    private void addCall2(int src, int dest, LinkedList<Integer> road, Elevator e) {
        ...
        boolean findSrc = false, finddest = false;
        int index = 0;
        while(!findSrc && index + 1 < road.size()){
            if (road.get(index) == null){
                road.addLast(src);
                findSrc = true;
                index++;
                break;
            }
            while(road.get(index) != null){
                if(road.get(index) > road.get(index+1) && road.get(index) >= src){
                    if (src >= road.get(index+1)){
                        road.add(index,src);
                        findSrc = true;
                        index++;
                        break;
                ...
                else if (road.get(index) < road.get(index+1) && road.get(index) <= src){
                    if (src <= road.get(index+1)){
                        road.add(index,src);
                        findSrc = true;
                        index++;
        ...
        int indexSrc = index;
        int floorDest = 0;
        while(!finddest && index + 1 < road.size()){
            if (road.get(index) == null){
                road.addLast(dest);
                finddest = true;
                index++;
    ...
    private double calcTime2(int src, int dest, LinkedList<Integer> road, Elevator e) {
        double time = 0;
        double timeSrc = 0, timeDest = 0;

        if (road.size() == 0){
            timeSrc = e.getStartTime() + (Math.abs(e.getPos()-src))/e.getSpeed() + e.getStopTime() + e.getTimeForOpen() + e.getTimeForClose();
            timeDest = e.getStartTime() + (Math.abs(dest-src))/e.getSpeed() + e.getStopTime() + e.getTimeForOpen() + e.getTimeForClose();
            ...
            while(road.get(index) != null ){
                ...
                if(road.get(index) > road.get(index+1) && road.get(index) >= src){
                    if (src >= road.get(index+1)){
                        findSrc = true;
                        if (index + 1 >= road.size())
                            floorSrc += Math.abs(road.get(index) - src);
                        else
                            floorSrc += Math.abs(road.get(index) - road.get(index+1));
                        index++;
                        break;
                    }
                    else {
        ...
        while(!finddest && index + 1 < road.size() ){
            if (road.get(index) == null){
                finddest = true;
                if (index + 1 >= road.size())
                    floorSrc += Math.abs(road.get(index) - dest);
                else
                    floorSrc += Math.abs(road.get(index) - road.get(index+1));
                index++;
                break;
        ...
        int indexDest = index - indexSrc;
        if(indexDest == 0)
            indexDest =1;
        timeSrc = floorSrc/e.getSpeed() + indexSrc*(e.getStopTime() + e.getStartTime() + e.getTimeForClose() + e.getTimeForOpen());
        timeDest = floorDest/e.getSpeed() + indexDest*(e.getStopTime() + e.getStartTime() + e.getTimeForClose() + e.getTimeForOpen());

        time = timeDest + timeSrc;
        return time;
    }
